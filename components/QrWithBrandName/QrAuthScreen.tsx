...
export default function QrAuthScreen({
  ...
  const CardLink = ({ gs }: ICardItemProps) => {
    ...
    const handleChannelClick = (channel: any) => {
      const gsUrl = channel?.isExpired ? `${channel?.groupshop?.url}/status&activated` : channel?.groupshop?.url;
      // @ts-ignore
      // eslint-disable-next-line no-undef
      fbq('trackCustom', 'QrSearch', {
        GSURL: gsUrl,
        Brand: channel?.shop?.brandName ? channel?.shop?.brandName : '',
    ...
    return (
      <div className={styles.cardItem__links}>
        <div>
          <Button
            onClick={() => handleChannelClick(gs)}
            className={styles.QRContainer__ViewGroupShop}
          >
