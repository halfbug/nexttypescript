...
import { GET_PARTNER_MOST_VIRAL_CUSTOMERS } from 'store/store.graphql';
import { read, utils, writeFileXLSX } from 'xlsx';
import useUtilityFunction from 'hooks/useUtilityFunction';
import AnalyticCustomerPartnerDetail from './AnalyticCustomerPartnerDetail';
import MostViralPartnerCustomers from './MostViralPartnerCustomers';
...
export default function CutomerPartnerData({
  ...
  } = useQuery(GET_PARTNER_MOST_VIRAL_CUSTOMERS, {
    variables: { storeId, startDate, endDate },
  });
  useEffect(() => {
    if (data?.partnerViralCustomers) {
      setShowCustomerDetail(false);
      setShowLineitemsBox(false);
      const myObjArray: { partnerName: string;
      revenueGenerated: string; uniqueClicks: any; discount: any;
      totalLineItems: any; Members: any; }[] = [];
      setViralCustomers(data.partnerViralCustomers);
